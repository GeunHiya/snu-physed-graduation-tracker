...
window.CourseInputRenderer = ({ i, ck, handlers, getForeign2Options, studentYear, data }) => {
    ...
    // 모바일(기본): w-full(가로 꽉 채움) -> 두 줄로 떨어짐
    // 데스크탑(md): w-auto -> 한 줄로 붙음
    const containerClass = "flex flex-col items-start gap-1.5 w-full md:flex-row md:items-center md:gap-2 text-sm font-bold";
    const selectClass = "bg-slate-100 px-3 py-2 rounded-xl focus:ring-2 focus:ring-indigo-100 outline-none font-bold w-full md:w-auto text-slate-700 text-xs md:text-sm";
    const inputClass = "bg-transparent border-none outline-none text-sm w-full font-bold input-underline focus:text-indigo-600 border-b border-transparent focus:border-transparent text-slate-700 py-1";
    // [신규] PC에서만 보이는 콜론 스타일
    const colonClass = "hidden md:block text-slate-400 shrink-0";

    if (i.id === 'g_be') {
        if (studentYear >= 24) return <div className={containerClass}><span className="text-slate-700 shrink-0">기초 영어</span></div>;
        return <div className={containerClass}><span className="text-slate-700 shrink-0">기초 영어</span><span className={colonClass}>:</span><select value={i.takenYear || 'before23'} onChange={(e) => handleBasicEnglishYearChange(ck, i.id, e.target.value)} className={selectClass}><option value="before23">2023학년도 이전 수강 (1pt)</option><option value="after24">2024학년도 이후 수강 (2pt)</option></select></div>;
    }
    if (i.type === 'foreign1') return <div className={containerClass}><span className="text-slate-400 shrink-0">외국어 1</span><span className={colonClass}>:</span><select value={i.subName} onChange={(e) => handleForeignChange(ck, i.id, 'subName', e.target.value)} className={selectClass}>{FOREIGN1_OPTIONS.map(opt => <option key={opt} value={opt}>{opt === '면제' ? '제2외국어' : opt}</option>)}</select>{i.subName === '면제' && <input type="text" value={i.name || ''} onChange={(e) => updateName(ck, i.id, e.target.value)} className={inputClass} placeholder="과목명..." />}</div>;
    if (i.type === 'foreign2') return <div className={containerClass}><span className="text-slate-400 shrink-0">외국어 2</span><span className={colonClass}>:</span><select value={i.subName} onChange={(e) => handleForeignChange(ck, i.id, 'subName', e.target.value)} className={selectClass}><option value="" disabled>선택</option>{getForeign2Options().map(opt => <option key={opt} value={opt}>{opt}</option>)}</select>{i.subName === '제2외국어' && <input type="text" value={i.name || ''} onChange={(e) => updateName(ck, i.id, e.target.value)} className={inputClass} placeholder="과목명..." />}</div>;
    ...
    // [과사실, 과학] 콜론 추가
    if (i.type === 'msSet') return <div className={containerClass}><span className="text-slate-400 shrink-0">{i.name}</span><span className={colonClass}>:</span><select value={i.subName} onChange={(e) => handleMSChange(ck, i.id, e.target.value)} className={selectClass}><option value={i.subName} disabled>{i.subName}</option>{((studentYear === 23 || studentYear === 24) ? MS_OPTIONS.filter(o => ['물리학 1', '화학 1', '생물학 1', '지구과학'].includes(o.main)) : MS_OPTIONS).map(o => <option key={o.main} value={o.main}>{o.main}</option>)}</select></div>;
    ...
    // [컴퓨터] 콜론 추가
    if (i.type === 'computer') return <div className={containerClass}><span className="text-slate-400 shrink-0 font-bold">{i.prefix}</span><span className={colonClass}>:</span><select value={i.name} onChange={(e) => updateName(ck, i.id, e.target.value)} className={selectClass}><option value="선택" disabled>선택</option>{i.options.map(opt => <option key={opt} value={opt}>{opt}</option>)}</select></div>;
    ...
    // [핵심교양] 콜론 추가
    if (i.type === 'core' || i.type === 'pe' || i.type === 'veritas') return <div className={containerClass}>{i.options ? <select value={i.prefix} onChange={e => handleCorePrefixChange(ck, i.id, e.target.value)} className={selectClass}>{i.options.map(opt => <option key={opt} value={opt}>{opt}</option>)}</select> : <span className="text-slate-400 shrink-0 font-bold">{i.prefix}</span>}<span className={colonClass}>:</span><input type="text" value={i.name} onChange={e => updateName(ck, i.id, e.target.value)} className={inputClass} placeholder="과목명..." /></div>;
    
    // [고정 핵심교양] 콜론 추가
    if (i.type === 'coreFixed') return <div className={containerClass}><span className="text-slate-400 shrink-0 font-bold">{i.prefix}</span><span className={colonClass}>:</span><input type="text" value={i.name} onChange={e => updateName(ck, i.id, e.target.value)} className={inputClass} placeholder="과목명..." /></div>;
    
    if (i.selectable) return <select value={i.name.includes('[') ? '' : i.name} onChange={(e) => handleSubjectSelect(ck, i.id, e.target.value)} className={`bg-slate-50 border-none outline-none text-sm font-bold text-slate-700 py-2 px-3 rounded-xl focus:ring-2 focus:ring-indigo-100 w-full ${i.completed ? 'text-slate-300 font-normal' : ''}`}><option value="" disabled>{i.name}</option>{PHYSICS_ED_CHOICES.map(opt => <option key={opt.name} value={opt.name}>{opt.name} ({opt.credits}pt)</option>)}</select>;
